...
void TestdeConstr() {
    ...
    NS_LOG_INFO("Test de construction...");
}
...
main(int argc, char* argv[])
{
    ...
    Ipv4GlobalRoutingHelper::PrintRoutingTableAllAt(Seconds(6.0), routingStream2);
    ...
    OnOffHelper onoff("ns3::TcpSocketFactory",
                      Address(InetSocketAddress(Ipv4Address("10.1.2.3"), port)));
    ...
    Simulator::Schedule(Seconds(5.0), [&]() {
        staticRoutingSnd->AddNetworkRouteTo(Ipv4Address("O.O.O.O"), Ipv4Mask("0.0.0.0"), Ipv4Address("192.168.1.3"), 1);
        staticRoutingSnd->RemoveRoute(2);
        staticRoutingRcv->AddNetworkRouteTo(Ipv4Address("O.O.O.O"), Ipv4Mask("0.0.0.0"), Ipv4Address("10.1.2.2"), 1);
        staticRoutingRcv->RemoveRoute(2);
    });
