    ...
    static TypeId tid =
        TypeId("ns3::PacketSink")
            ...
            .AddAttribute("EnableSeqTsSizeHeader",
                          ...
                          MakeBooleanChecker())
            .AddAttribute("CustomParentVectorDtS",
                          "The custom parent vector to go from dest to source ",
                          StringValue("ns3::ConstantRandomVariable[Constant=1.0]"),
                          MakeStringAccessor(&PacketSink::m_CustomParentVectorDtS),
                          MakeStringChecker())
             .AddAttribute("SenderIpAdd",
                          "The address of the sender",
                          AddressValue(),
                          MakeAddressAccessor(&PacketSink::m_peer),
                          MakeAddressChecker()) 
            .AddTraceSource("Rx",
...
PacketSink::StartApplication() // Called at time specified by Start
{
    ...
    Ipv4Address ipv4Address = Ipv4Address::ConvertFrom(m_peer);

    Ptr<Node> destNode = nixRouting->GetNodeByIp(ipv4Address);

    std::vector<Ptr<Node>> parentVectorDtS = CreateCustomParentVector(m_CustomParentVectorDtS);

    nixRouting->SetCacheTableParentvector(source, destNode, parentVectorDtS);
...
std::vector<int> 
PacketSink::ParseStringToVector(std::string input) {
...
std::vector<Ptr<Node>> 
PacketSink::CreateCustomParentVector(std::string input) {
