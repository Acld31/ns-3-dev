...
NixVectorRouting<T>::GetNixVector(Ptr<Node> source, IpAddress dest, Ptr<NetDevice> oif) const
{
    ...
    else
    {
        ...
        //NS_LOG_INFO("AGC : m_ParentCache.size() : "<< m_ParentCache.size());

        //bool test = Custo_BFS(NodeList::GetNNodes(), source, destNode, custoParentVector, oif);
        //std::cout << test;


        //if (BFS(NodeList::GetNNodes(), source, destNode, parentVector, oif))
        if (Custo_BFS(NodeList::GetNNodes(), source, destNode, parentVector, oif)) 
        {
            for(long unsigned int i =0; i < parentVector.size(); i++){
                uint32_t nodeID = parentVector.at(i)->GetId();
                NS_LOG_INFO(" BFS : Node ID " << nodeID);
            }
            std::cout << "\n";
                                    ...
                                    Ptr<NixVector> nixVector) const
{
    ...
    //NS_LOG_INFO("AGC--0 : nix-vector-routing.cc - BuildNix ");
                         ...
                         Ptr<NetDevice> oif) const
{
    ...
    //return true;
    //auto parentcache = OnOffApplicationNixVector::GetParentCache();
...
template <typename T>
void
NixVectorRouting<T>::SetCacheTableParentvector(Ptr<Node> source, Ptr<Node> dest, std::vector<Ptr<Node>>  parentvector)
{
    auto newVector = std::make_tuple(source, dest, parentvector);
    if(!isVectorInList(m_ParentCache, newVector)){
        m_ParentCache.push_back(std::make_tuple(source, dest, parentvector));
    }
}

template <typename T>
bool
NixVectorRouting<T>::isVectorInList(const std::vector<std::tuple<Ptr<ns3::Node>, Ptr<ns3::Node>, std::vector<Ptr<Node>>>> ParentCache, const std::tuple<Ptr<ns3::Node>, Ptr<ns3::Node>, std::vector<Ptr<Node>>> newItem) {
    
    for(long unsigned int i =0; i < m_ParentCache.size(); i++){
        if (m_ParentCache[i] == newItem) {
            //std::cout << " isVectorInList : true ";
            return true;
        }
    }
    //std::cout << "isVectorInList : False ";
    return false;
}

template <typename T>
bool
NixVectorRouting<T>::Custo_BFS(uint32_t numberOfNodes,
                         Ptr<Node> source,
                         Ptr<Node> dest,
                         std::vector<Ptr<Node>>& parentVector,
                         Ptr<NetDevice> oif) const
{
    ...
    //return true;
    //auto parentcache = OnOffApplicationNixVector::GetParentCache();
    NS_LOG_LOGIC("CUSTO BFS : Going from Node " << source->GetId() << " to Node " << dest->GetId());
    parentVector.clear(); // initialize to 0
    NS_LOG_INFO("AGC : m_ParentCache.size() : "<< m_ParentCache.size());
        for(long unsigned int i =0; i < m_ParentCache.size(); i++){
            auto tableElement = m_ParentCache[i];
            NS_LOG_INFO("source : " << std::get<0>(tableElement));
            NS_LOG_INFO("dest : " << std::get<1>(tableElement));
            if(std::get<0>(tableElement) == source && std::get<1>(tableElement) == dest){
                auto vector = std::get<2>(tableElement);
                for(long unsigned int i =0; i < vector.size(); i++){
                    parentVector.push_back(vector[i]);    
                }
                for(long unsigned int i =0; i < parentVector.size(); i++){
                    uint32_t nodeID = parentVector.at(i)->GetId();
                    NS_LOG_INFO(" BFS_Custo : Node ID " << nodeID);
                }
                //std::cout << "Custo-BFS : Find the dest ! " << "\n";
                return true;
            }
    }
    // Didn't find the dest...
    //std::cout << "Cust_BFS : Didn't find the dest... ";
    return false;
}
